...
# Because torchscript cannot handle callables had to make mlp it's own module
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class GPT(nn.Module):
    ...
    def __init__(self, config, export_mode=False):
        super().__init__()
    ...
    def forward(self, idx):
        ...
        pos = torch.arange(
            0, t, dtype=torch.long, device=device, layout=None
        ).unsqueeze(
            0
        )  # shape (1, t)
